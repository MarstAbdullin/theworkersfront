    ...
    <div class="container">
        <header class="jumbotron">
            ...
            <div v-if="userInfo.teachers.length">
                <h5>Преподаватели:</h5>
                <ul>
                    <li v-for="teacher in userInfo.teachers" v-bind:key="teacher.id">{{teacher.firstName}}
                        {{teacher.secondName}}
                    </li>
                </ul>
            </div>
            ...
            <div v-if="userInfo.resumes">
                <h5>Резюме:</h5>
                <ul>
                    <li v-for="obj in userInfo.resumes" v-bind:key="obj.id">
                        <a :href="'/resume/' + obj.id">{{obj.resumeName}}</a>
                        <p><a :href="'/changeResume/' + obj.id">Поменять {{obj.resumeName}} резюме</a></p>
                    </li>
                </ul>
            </div>
        </header>
